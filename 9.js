// 1. Configuración y selección de elementos del DOM
...
// 2. Mostrar las iniciales
function mostrarIniciales() {
    inicialesContainer.innerHTML = ""; // Limpiar el contenedor
...
// 3. Mostrar listado por inicial

// Función para mostrar el listado de antiinfecciosos según la inicial seleccionada
function mostrarListadoPorInicial(letra) {
    const lista = antibiotics[letra];
    if (!lista || lista.length === 0) {
        listadoContainer.innerHTML = `<p>No se encontraron resultados para la letra "${letra}".</p>`;
    } else {
        listadoContainer.innerHTML = lista.map(antibiotico => `
            <div class="aii-item" onclick='mostrarDetalles(${JSON.stringify(antibiotico)})'>
                ${antibiotico.name}
            </div>
        `).join("");
    }
    toggleVista(false); // Cambiar la vista para ocultar las iniciales y mostrar el listado
}




// 4. Mostrar detalles del antiinfeccioso seleccionado

function mostrarDetalles(antibiotico) {
    // Actualiza cada celda con la información correspondiente
    function mostrarDetalles(antibiotico) {
        document.querySelector("#presentation").textContent = antibiotico.presentation;
        document.querySelector("#type").textContent = antibiotico.type;
        document.querySelector("#dose").textContent = antibiotico.dose;
        document.querySelector("#preparation").textContent = antibiotico.preparation;
        document.querySelector("#appearance").textContent = antibiotico.appearance;
        document.querySelector("#administration-time").textContent = antibiotico.administrationTime;
        document.querySelector("#storage").textContent = antibiotico.storage;
        document.querySelector("#technicalSheet").setAttribute("href", antibiotico.technicalSheet);
        ...
        console.log("Antibiotico seleccionado:", antibiotico);
...
// 5. Buscar antiinfeccioso
buscarInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
        const searchText = buscarInput.value.toLowerCase();
        const resultados = Object.values(antibiotics).flat().filter(antibiotico => antibiotico.name.toLowerCase().includes(searchText));

        if (resultados.length > 0) {
            listadoContainer.innerHTML = resultados.map(item => `<div class='aii-item' onclick='mostrarDetalles("${item.name}", "${item.details}")'>${item.name}</div>`).join("");
        } else {
            listadoContainer.innerHTML = `<p>No se encontraron resultados para "${searchText}".</p>`;
        }
        toggleVista(false);
    }
});

// 6. Volver a la vista inicial
...
function toggleVista(mostrarBusqueda) {
    zonaBusqueda.style.display = mostrarBusqueda ? "block" : "none";
