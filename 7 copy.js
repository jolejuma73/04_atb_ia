...
//ocultar contenedor 
function toggleSidebarSeekWrapper(shouldShow) {
    const wrapper = document.querySelector('.sidebar-seek-wrapper');
    if (wrapper) {
        wrapper.style.display = shouldShow ? 'flex' : 'none'; // Mostrar u ocultar
    }
}

// Ocultar el contenedor al seleccionar una letra
document.getElementById('alphabet-list').addEventListener('click', (event) => {
    if (event.target.tagName === 'LI') {
        toggleSidebarSeekWrapper(false); // Ocultar
    }
});

// Mostrar el contenedor al reiniciar la búsqueda
document.getElementById('main-button').addEventListener('click', () => {
    toggleSidebarSeekWrapper(true); // Mostrar
});
