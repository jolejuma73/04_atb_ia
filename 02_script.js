...
document.getElementById("reset-button").addEventListener("click", function () {
    ...
    // Restaurar resultados de búsqueda
    const searchResults = document.getElementById("search-results");
    searchResults.innerHTML = "";

    // Restaurar información del antibiótico
    const antibioticInfo = document.getElementById("antibiotic-info");
    antibioticInfo.innerHTML = `
    ...
    // Regenerar alfabeto si es necesario (opcional)
    const alphabetList = document.getElementById("alphabet-list");
    alphabetList.innerHTML = ""; // Borra la lista existente
    for (let i = 65; i <= 90; i++) {
        const letter = String.fromCharCode(i);
        const listItem = document.createElement("li");
        listItem.textContent = letter;
        ...
        // Vuelve a agregar eventos a las letras
        listItem.addEventListener("click", () => displayAntibiotics(letter));
        alphabetList.appendChild(listItem);
    }
});
